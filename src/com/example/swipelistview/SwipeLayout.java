...
public class SwipeLayout extends FrameLayout implements SwipeLayoutInterface{
	...
	ViewDragHelper.Callback mCallback = new ViewDragHelper.Callback() {
		...
		public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
			...
			if(getFrontView().getLeft()==-mDragDistance && mStatus!=Status.Open){
				mStatus = Status.Open;
				
				//说明当前的SwipeLayout已经打开了，那么就需要让manager去记录
				SwipeLayoutManager.getInstance().setSwipeLayout(SwipeLayout.this);
				
			}else if (getFrontView().getLeft()==0 && mStatus!=Status.Close) {
	...
	public boolean onInterceptTouchEvent(android.view.MotionEvent ev) {
		...
		// 返回true则为拦截
		boolean result = mDragHelper.shouldInterceptTouchEvent(ev)& mGestureDetector.onTouchEvent(ev);
